...
- **Не использовать шаблон «это не просто X, это Y»** в заголовках и вводных предложениях — он звучит снисходительно и не несёт информации. Пиши сразу что это такое, без предисловий.
- **Для статей уровня «новичок»**: не использовать технический жаргон без объяснения. Любой термин должен быть либо объяснён простыми словами, либо заменён аналогией. Пиши так, будто объясняешь человеку, который видит тему впервые — никаких «userspace-приложение», «JIT-компиляция», «AST» без вводного объяснения на человеческом языке.
